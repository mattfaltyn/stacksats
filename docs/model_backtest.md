...
The backtest system validates the dynamic DCA strategy by comparing its performance against uniform DCA (equal daily allocations) across rolling windows using the configured fixed allocation span (default: 365 days) from 2018-01-01 to the present.
...
flowchart TD
    ...
    subgraph backtest [Backtest Engine]
        WINDOWS[Rolling Fixed-Span Windows<br/>Daily start dates]
...
For each daily start date from 2018-01-01, creates an inclusive fixed-span investment window (default: 365 days):
...
|--------|-------|-------------|
| Windows tested | 2,554 | All rolling fixed-span windows in the selected range |
...
- Window generation enforces the configured fixed allocation span in `stacksats/prelude.py`.
- Contract-level guards (finite/range/immutability/final invariants) are centralized in `stacksats/framework_contract.py`.
- Backtest parity and lock semantics are tested in `tests/integration/backtest/test_backtest_export_parity.py` and `tests/unit/model/test_weight_stability.py`.
